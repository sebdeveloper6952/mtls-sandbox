...
func TestAddCallAndList(t *testing.T) {
	...
	callID, err := s.AddCall(sess.ID, "https://example.com", TestModeNormal, "GET", 200, 42, "", probe)
	if err != nil {
		t.Fatal(err)
	}
	if callID <= 0 {
	...
	// Add a failed call.
	_, err = s.AddCall(sess.ID, "https://fail.example.com", TestModeWrongCA, "GET", 0, 100, "connection refused", nil)
	if err != nil {
		t.Fatal(err)
	}
